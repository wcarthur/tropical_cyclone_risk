...
def gen_f(N, T, t, num):
    ...
    for i in range(0, num):
        ...
        xln = np.tile(np.random.rand(N, 1), (1, np.size(t)))
...
class BetaAdvectionTrack:
    ...
    def __init__(self, fn_wnd_stat, basin, dt_start, dt_track=3600,
        ...
        self.t_s = np.linspace(0, self.total_time, int(
            self.total_time / self.dt_track) + 1)
        ...
        for i in range(self.nLvl):
            self.u_Mean_idxs[i] = int(self.var_names.index(
                'ua' + str(p_lvls[i]) + '_Mean'))
            self.v_Mean_idxs[i] = int(self.var_names.index(
                'va' + str(p_lvls[i]) + '_Mean'))
        self._load_wnd_stat()

    def _interp_basin_field(self, var):
        lon_b, lat_b, var_b = self.basin.transform_global_field(
            self.wnd_lon, self.wnd_lat, var)
    ...
    def _load_wnd_stat(self):
        ...
        self.datetime_start = input.convert_to_datetime(
            ds, np.array([self.dt_start]))
        ...
        for i in range(len(wnd_Mean)):
            self.wnd_Mean_Fxs[i] = self._interp_basin_field(
                wnd_Mean[i].interp(time=self.dt_start))
