...
def get_fn_thermo():
    """
    Retrieve the filename of the thermodynamic data file

    :return: Formatted file name
    :rtype: str
    """
    fn_th = "%s/thermo_%s_%d%02d_%d%02d.nc" % (
...
def compute_thermo(dt_start, dt_end):
    """
    Compute thermodynamic values - potential intensity, chi and mid-level
    relative humidity
    ...
    :return: Arrays of vmax, chi and rh_mid
    ...
    for i in range(nTime):
        ...
        rh_mid[i, :, :] = thermo.conv_q_to_rh(
            ta_midlevel, hus_midlevel, namelist.p_midlevel
        )
...
def gen_thermo():
    ...
    for i in range(n_chunks):
        lazy_result = dask.delayed(compute_thermo)(
